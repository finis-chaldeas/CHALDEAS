...
class MatchResult:
    ...
    confidence: float
    method: str  # 'exact', 'alias', 'wikidata', 'embedding', 'llm', 'partial', 'new'
    entity_name: Optional[str] = None  # 매칭된 엔티티의 실제 이름
...
class EntityMatcher:
    ...
    def __init__(self, auto_create: bool = False, fast_mode: bool = False, wikidata_only: bool = False):
        """
        Args:
            auto_create: True면 매칭 실패 시 자동으로 새 엔티티 생성
            fast_mode: True면 로컬 DB만 사용 (exact → alias → trigram)
                       외부 API 호출 안 함 (Wikidata, OpenAI, Ollama)
            wikidata_only: True면 wikidata_id 있는 엔티티만 매칭 (V2 모드)
        """
        self.auto_create = auto_create
        self.fast_mode = fast_mode
        self.wikidata_only = wikidata_only
        self._conn = None
    ...
    def match(self, entity_type: str, name: str, context: str = None) -> MatchResult:
        ...
        파이프라인:
            fast_mode=True:  exact → alias → trigram
            fast_mode=False: exact → alias → wikidata → embedding → llm → trigram
        """
        ...
        if entity:
            return MatchResult(True, entity['id'], 1.0, 'exact', entity_name=entity['name'])

        # 2. Alias match
        entity = self._alias_match(entity_type, name)
        if entity:
            return MatchResult(True, entity['id'], 0.95, 'alias', entity_name=entity['name'])

        # Fast mode: 외부 API 스킵, 바로 trigram으로
        ...
        # 6. Partial match (trigram fuzzy)
        partial_result = self._partial_match(entity_type, name)
        if partial_result:
            self._save_alias(entity_type, partial_result['id'], name, 'partial')
            return MatchResult(True, partial_result['id'], 0.88, 'partial',
                             entity_name=partial_result['name'])

        # No match
        if self.auto_create:
            new_entity = self._create_entity(entity_type, name, qid if not self.fast_mode else None)
            return MatchResult(True, new_entity['id'], 1.0, 'new', entity_name=name)

        return MatchResult(False, None, 0.0, 'none', details={'wikidata_qid': qid if not self.fast_mode else None})
    ...
    def _exact_match(self, table: str, name: str) -> Optional[Dict]:
        ...
        wikidata_filter = "AND wikidata_id IS NOT NULL" if self.wikidata_only else ""
        cur.execute(f"""
            SELECT id, {name_col} as name, wikidata_id
            FROM {table}
            WHERE LOWER({name_col}) = LOWER(%s) {wikidata_filter}
            LIMIT 1
        """, (name,))
        return cur.fetchone()
    ...
    def _alias_match(self, entity_type: str, name: str) -> Optional[Dict]:
        ...
        table = self._get_table(entity_type)
        name_col = 'title' if table == 'events' else 'name'
        wikidata_filter = "AND e.wikidata_id IS NOT NULL" if self.wikidata_only else ""

        # wikidata_only 모드면 entity와 조인해서 필터
        cur.execute(f"""
            SELECT ea.entity_id
            FROM entity_aliases ea
            JOIN {table} e ON e.id = ea.entity_id
            WHERE ea.entity_type = %s AND LOWER(ea.alias) = LOWER(%s) {wikidata_filter}
            LIMIT 1
        """, (entity_type, name))
        result = cur.fetchone()

        if result:
            cur.execute(f"SELECT id, {name_col} as name, wikidata_id FROM {table} WHERE id = %s",
                       (result['entity_id'],))
            return cur.fetchone()
        return None

    # ─── Stage 6: Fuzzy Match (Trigram Similarity) ─────────────

    def _partial_match(self, entity_type: str, name: str) -> Optional[Dict]:
        """
        Trigram 유사도 기반 퍼지 검색 (GIN 인덱스 활용)

        1. Alias 테이블에서 유사도 검색 (% 연산자 - GIN 인덱스)
        2. Entity 테이블에서 직접 유사도 검색 (% 연산자 - GIN 인덱스)
        3. 가장 높은 유사도 결과 반환

        장점:
        - 오타 허용: "Napolean" → "Napoleon"
        - Title 자동 처리: "King Minos" → "Minos" (alias에서 찾음)
        - 언어 변형: "Aleksander" → "Alexander"
        - GIN 인덱스 사용으로 빠름 (50ms vs 6초)
        """
        table = self._get_table(entity_type)
        cur = self.conn.cursor(cursor_factory=RealDictCursor)
        name_col = 'title' if table == 'events' else 'name'
        wikidata_filter = "AND e.wikidata_id IS NOT NULL" if self.wikidata_only else ""
        wikidata_filter_direct = "AND wikidata_id IS NOT NULL" if self.wikidata_only else ""

        best_match = None
        best_similarity = 0.0

        # Set similarity threshold (used by % operator)
        cur.execute("SET pg_trgm.similarity_threshold = 0.5")

        # 1. Alias 테이블에서 trigram 검색 (% 연산자 - GIN 인덱스 활용)
        cur.execute(f"""
            SELECT ea.entity_id, ea.alias, similarity(ea.alias, %s) as sim
            FROM entity_aliases ea
            JOIN {table} e ON e.id = ea.entity_id
            WHERE ea.entity_type = %s AND ea.alias %% %s {wikidata_filter}
            ORDER BY sim DESC
            LIMIT 1
        """, (name, entity_type, name))
        alias_result = cur.fetchone()

        if alias_result and alias_result['sim'] > best_similarity:
            # 해당 entity 정보 가져오기
            cur.execute(f"SELECT id, {name_col} as name, wikidata_id FROM {table} WHERE id = %s",
                       (alias_result['entity_id'],))
            entity = cur.fetchone()
            if entity:
                best_match = entity
                best_similarity = alias_result['sim']

        # 2. Entity 테이블에서 직접 trigram 검색 (% 연산자 - GIN 인덱스 활용)
        cur.execute(f"""
            SELECT id, {name_col} as name, wikidata_id, similarity({name_col}, %s) as sim
            FROM {table}
            WHERE {name_col} %% %s {wikidata_filter_direct}
            ORDER BY sim DESC
            LIMIT 1
        """, (name, name))
        direct_result = cur.fetchone()

        if direct_result and direct_result['sim'] > best_similarity:
            best_match = {
                'id': direct_result['id'],
                'name': direct_result['name'],
                'wikidata_id': direct_result['wikidata_id']
            }
            best_similarity = direct_result['sim']
