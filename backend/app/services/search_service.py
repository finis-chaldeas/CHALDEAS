...
def search_all(
    ...
    if type_filter in (None, "all", "event"):
        event_query = db.query(Event).outerjoin(EventDetail).filter(
            Event.wikidata_id.isnot(None),  # V2 모드
            or_(
                Event.title.ilike(search_term),
                Event.title_ko.ilike(search_term),
                EventDetail.description.ilike(search_term),
            )
        )
    ...
    if type_filter in (None, "all", "person"):
        person_query = db.query(Person).filter(
            Person.wikidata_id.isnot(None),  # V2 모드
