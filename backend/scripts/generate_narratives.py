"""
Generate AI-powered historical narratives for 50-year periods.

Breaks each period into regional sub-narratives + a global overview.
Regions: europe, near_east, south_asia, east_asia, africa, americas.

Uses gpt-5.1-chat-latest for narrative generation and Wikipedia REST API
for supplementary context.

Usage:
    cd backend
    python scripts/generate_narratives.py                  # Full run
    python scripts/generate_narratives.py --test 3         # Test with 3 periods
    python scripts/generate_narratives.py --resume         # Resume from checkpoint
    python scripts/generate_narratives.py --apply          # Apply JSONL to DB
    python scripts/generate_narratives.py --stats          # Show checkpoint stats

Output: data/compact_export/era_narratives.jsonl
"""
import argparse
import json
import os
...
CHECKPOINT_FILE = CHECKPOINT_DIR / "era_narratives.jsonl"
...
# --- Regions ---
REGIONS = [
    {"id": "europe", "name": "Europe & Mediterranean",
     "lat_range": (35, 72), "lng_range": (-15, 40)},
    {"id": "near_east", "name": "Near East & Central Asia",
     "lat_range": (15, 50), "lng_range": (25, 70)},
    {"id": "south_asia", "name": "South Asia",
     "lat_range": (5, 40), "lng_range": (60, 100)},
    {"id": "east_asia", "name": "East Asia & Pacific",
     "lat_range": (10, 55), "lng_range": (100, 150)},
    {"id": "africa", "name": "Africa",
     "lat_range": (-35, 35), "lng_range": (-20, 55)},
    {"id": "americas", "name": "Americas",
     "lat_range": (-55, 70), "lng_range": (-170, -30)},
]


def _region_case_sql(lat_col="l.latitude", lng_col="l.longitude"):
    """Build SQL CASE for classifying coordinates into regions."""
    parts = []
    for r in REGIONS:
...
def get_regional_data(db, period_start, period_end):
    ...
    Events/persons without locations are distributed to the most relevant region
    based on their content, rather than being grouped into a 'global' pseudo-region
    (which would overlap with real regional narratives).
    """
    # Events WITH location -> classified by region
    ...
    # Events WITHOUT location — fold into the highest-scoring region
    # or the first region with events (avoids creating a separate overlapping "global" region)
    no_loc_events = db.execute(text("""
        ...
        WHERE e.date_start >= :start AND e.date_start <= :end
          ...
          AND (e.primary_location_id IS NULL
               OR e.primary_location_id NOT IN (
                   SELECT id FROM locations WHERE latitude IS NOT NULL))
        ORDER BY e.importance_score DESC LIMIT 5
    """), {"start": period_start, "end": period_end, "min_score": MIN_SCORE}).fetchall()

    # Persons WITH location -> classified by region
    ...
    # Fold locationless events into the top-scoring region
    if no_loc_events and regions:
        top_region = max(regions.keys(), key=lambda r: max(
            [e["importance_score"] for e in regions[r]["events"]] +
            [p["score"] for p in regions[r]["persons"]] + [0]
        ))
        for row in no_loc_events:
            if len(regions[top_region]["events"]) < 7:
                regions[top_region]["events"].append({
                    "id": row[0], "title": row[1], "date_start": row[2],
                    "importance_score": row[3], "description": row[4],
                    "wikipedia_url": row[5], "location_name": None,
                })

    # Filter: keep regions with at least 1 entity scoring >= MIN_SCORE
    filtered = {}
    for rid, data in regions.items():
        total_items = len(data["events"]) + len(data["persons"])
        max_score = max(
            [e["importance_score"] for e in data["events"]] +
            [p["score"] for p in data["persons"]] + [0]
        )
...
def generate_regional_narrative(client, period, region_id, events, persons):
    ...
    for e in events[:3]:
        ...
        event_lines.append(
            f"  - {e['title']} ({format_year(e['date_start'])}) "
            f"[score:{e['importance_score']}] {loc} - {desc}"
        )
    ...
    for p in persons[:3]:
        ...
        person_lines.append(
            f"  - {p['name']} ({lifespan}) [{p.get('domain', '?')}] - {bio}"
        )
...
- Focus on this region specifically — do NOT cover other regions
- Explain WHY these developments matter in world history context
- If the database has few entries, supplement with your own knowledge of this region
- Mention connections to other regions only briefly if relevant

Also provide:
- A headline for this region during this period (max 80 chars)
- 2-4 keywords
- A defining quote from this era/region if one exists (with attribution), or leave empty
...
# --- DB Apply ---
def apply_to_db(db):
    """Upsert checkpoint entries into period_narratives table."""
    ...
    for line in CHECKPOINT_FILE.read_text(encoding='utf-8').splitlines():
        ...
        if entry["type"] in ("regional", "global_overview"):
            ...
            data = {
                ...
                "quote_source": entry.get("quote_source") or None,
            ...
            if existing:
                db.execute(text("""
                    ...
                    SET headline=:headline, narrative=:narrative, keywords=:keywords,
                        defining_moment=:defining_moment, quote=:quote, quote_source=:quote_source,
                        era_id=:era_id, model_used=:model_used,
                ...
                db.execute(text("""
                    INSERT INTO period_narratives
                        (period_start, period_end, region, era_id, headline, narrative,
                         keywords, defining_moment, quote, quote_source, model_used,
                         event_count, person_count, top_events, top_persons)
...
def run_pipeline(test_limit=None, resume=True):
    ...
    if test_limit:
        # Pick interesting periods (Classical Greece/Persia/Buddha/Confucius era)
    ...
    for i, period in enumerate(periods):
        ...
        # Get regional data (locationless events folded into top region)
        ...
        # --- Generate per-region narratives ---
        for rid in active_regions:
            ...
            if result:
                ...
                top_ev = [{"title": e["title"], "date": format_year(e["date_start"]),
                           "score": e["importance_score"]} for e in events[:3]]
                top_pe = [{"name": p["name"], "domain": p.get("domain"),
                           "score": p["score"]} for p in persons[:3]]
    ...
    print(f"\nTo apply: cd backend && python scripts/generate_narratives.py --apply")
    db.close()
...
def main():
    parser = argparse.ArgumentParser(description="Generate era narratives (regional breakdown)")
    parser.add_argument("--test", type=int, help="Test with N periods")
    parser.add_argument("--resume", action="store_true", default=True)
    parser.add_argument("--no-resume", action="store_true")
    parser.add_argument("--apply", action="store_true", help="Apply JSONL checkpoint to DB")
